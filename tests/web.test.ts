...
describe("Web Search API", () => {
  ...
  test("should work with different safe search levels", async () => {
    const client = createJigsawStackClient();
    const safeSearchLevels = ["strict", "moderate", "off"];
...
// Deep Research API Tests (lightweight tests to avoid long execution times)
describe("Deep Research API", () => {
  ...
  test("should work with minimal deep_research configuration", async () => {
    const result = await client.web.search({
      query: "simple test",
      deep_research: true,
      deep_research_config: {
        max_depth: 1, // Minimal depth to reduce execution time
        max_breadth: 1, // Minimal breadth to reduce execution time
        max_output_tokens: 1000, // Minimal tokens to reduce execution time
      },
    });
  ...
  test("should validate deep_research_config parameter structure", async () => {
  ...
  test("should work with deep_research enabled but no config (uses defaults)", async () => {
    const result = await client.web.search({
      query: "quick test", // Use simple query to minimize execution time
      deep_research: true,
      // No deep_research_config - should use defaults
    });
  ...
  test("should handle invalid deep_research_config values", async () => {
    try {
      await client.web.search({
        query: "test",
        deep_research: true,
      ...
      throw new Error("Expected API call to fail with invalid max_depth");
    } catch (error) {
      expectType(error, "object");
    }
  });

  test("should work with deep_research disabled", async () => {
    const result = await client.web.search({
      query: "normal search",
      deep_research: false,
    });
